...
class Game:
    def __init__(self):
        ...
        self.bosses = []
        self.projectiles = []
        self.pickups = []  # For drops
        self.view_mode = '2d_top'  # Default: 2D top-down; options: '1st', '3rd', 'helicopter', '2d_top', '2d_side'
        ...
        # Aim dot for 2d_top
        self.aim_dot_pos = [0, 0]  # Screen coords
        self.aim_dot_world = [0, 0]  # World x,z
    ...
    def mouse_motion(self, event):
        ...
        if self.view_mode == '2d_top':
            # Update aim dot
            self.aim_dot_pos = (event.x, event.y)
            # Convert to world (simple approx)
            scale = 5
            offset_x, offset_y = self.width // 2, self.height // 2
            aim_x = (event.x - offset_x) / scale + self.cam_pos[0]
            aim_z = (event.y - offset_y) / scale + self.cam_pos[2]
            self.aim_dot_world = [aim_x, aim_z]
        self.center_mouse()
    ...
    def show_pause_menu(self):
        ...
        view_menu = tk.OptionMenu(scrolled, view_var, '1st', '3rd', 'helicopter', '2d_top', '2d_side', command=self.set_view_mode)
        view_menu.pack()
    ...
    def generate_boss(self):
        ...
        pos = [random.uniform(-80, 80), self.eye_height, random.uniform(10, 40)]  # On ground
        ...
        boss = {
            ...
            'anim_time': 0  # For boss animations
        }
        self.bosses.append(boss)

    def do_normal_attack(self):
        self.attack_time = time.time() + 0.5  # Animation duration
        # Range-based
    ...
    def do_dodge(self):
        self.dodge_time = time.time() + 0.5  # Animation
        self.add_particles(self.cam_pos, 'white')  # Dust
        # Roll: add vel in dir
    ...
    def do_jump(self):
        if self.on_ground:
            self.jump_time = time.time() + 0.5  # Animation
            self.add_particles(self.cam_pos, 'gray')  # Jump dust
            self.vel_y = self.jump_speed
            self.stamina -= 10

    def do_charged_defense(self):
        self.defend_time = time.time() + 0.5  # Animation
    ...
    def on_boss_kill(self, idx):
        # Add pickup
        pos = self.bosses[idx]['pos'][:]
        pos[1] = self.eye_height
        drop_type = 'armor' if self.level == 0 else random.choice(self.progression)
        self.pickups.append({'pos': pos, 'type': drop_type, 'start_time': time.time(), 'lifetime': 30.0})
        del self.bosses[idx]
    ...
    def update(self):
        ...
        for boss in self.bosses:
            if current > boss['attack_cooldown']:
                ...
                if dist3d < 14:
                    ...
                    boss['anim_time'] = current + 0.5  # Boss attack animation
        ...
        # Pickups update (collect if close)
        new_pickups = []
        for pickup in self.pickups:
            if current - pickup['start_time'] < pickup['lifetime']:
                dist = sqrt((self.cam_pos[0] - pickup['pos'][0])**2 + (self.cam_pos[2] - pickup['pos'][2])**2)
                if dist < 2:
                    if pickup['type'] == 'armor':
                        self.armor += 10
                    elif pickup['type'] == 'weapon':
                        self.next_weapon = random.choice(['sword', 'axe', 'gun', 'bow', 'magic'])
                    elif pickup['type'] == 'skill_point':
                        self.skill_level += 1
                else:
    ...
    def render(self):
        ...
        if self.view_mode == '1st':
            cam_pos[1] = self.eye_height + 1.0  # Above ground
            render_player = False
        elif self.view_mode == '3rd':
            # Behind player, pinned
            cam_pos[0] -= sin(yaw) * 10
            cam_pos[2] -= cos(yaw) * 10
            cam_pos[1] += 2
            # Adjust to center player below middle
            self.center_y = self.height * 0.6  # Below center
        elif self.view_mode == 'helicopter':
            # High above, track player
            cam_pos[0] = self.cam_pos[0]
            cam_pos[2] = self.cam_pos[2]
            cam_pos[1] = 50
        ...
        # For 2d_top and 2d_side: flat scale
            ...
            if self.view_mode == '2d_top':
                ...
                player_fill = self.player_color
                player_size = 5
                if time.time() < self.jump_time:
                    player_fill = 'yellow'  # Jump flash
                elif time.time() < self.dodge_time:
                    player_size = 7  # Dodge expand
                elif time.time() < self.attack_time:
                    player_fill = 'red'  # Attack flash
                elif time.time() < self.defend_time:
                    player_fill = 'cyan'  # Defend flash
                self.canvas.create_oval(px - player_size, pz - player_size, px + player_size, pz + player_size, fill=player_fill)
                # Direction indicator
                dir_len = 10
                dx = sin(yaw) * dir_len
                dz = cos(yaw) * dir_len
                self.canvas.create_line(px, pz, px + dx * scale, pz + dz * scale, fill='white', width=2)
                # Eye dot
                eye_dx = sin(yaw) * (player_size - 1)
                eye_dz = cos(yaw) * (player_size - 1)
                self.canvas.create_oval(px + eye_dx - 1, pz + eye_dz - 1, px + eye_dx + 1, pz + eye_dz + 1, fill='black')
                # Aim dot
                if self.aim_dot_pos:
                    ax, ay = self.aim_dot_pos
                    self.canvas.create_oval(ax - 2, ay - 2, ax + 2, ay + 2, fill='white')
                # Bosses
                for boss in self.bosses:
                    bx = offset_x + boss['pos'][0] * scale
                    bz = offset_y + boss['pos'][2] * scale
                    boss_fill = boss['color']
                    boss_size = boss['radius'] * scale
                    if time.time() < boss.get('anim_time', 0):
                        boss_fill = 'orange'  # Boss attack flash
                    self.canvas.create_oval(bx - boss_size, bz - boss_size, bx + boss_size, bz + boss_size, outline=boss_fill)
                    # Health ring
                    health_ratio = boss['health'] / boss['max_health']
                    self.canvas.create_arc(bx - boss_size, bz - boss_size, bx + boss_size, bz + boss_size, start=0, extent=360*health_ratio, style='arc', outline='red', width=2)
                    # Glow if element
                    if boss['health'] < boss['max_health'] / 2:
                        glow_color = elem_glow_map.get(boss['element'], 'white')
                        self.canvas.create_oval(bx - boss_size - 2, bz - boss_size - 2, bx + boss_size + 2, bz + boss_size + 2, outline=glow_color)
                # Projectiles etc.
                for proj in self.projectiles:
                ...
                for pickup in self.pickups:
                    ...
                    pz = offset_y + pickup['pos'][2] * scale
                    self.canvas.create_oval(px - 3, pz - 3, px + 3, pz + 3, fill='gold')
                    # Bloom particles
                    for _ in range(5):
                        bx = px + random.uniform(-5, 5)
                        bz = pz + random.uniform(-5, 5)
                        self.canvas.create_oval(bx - 1, bz - 1, bx + 1, bz + 1, fill='yellow')
                return  # Skip 3D
            else:  # 2d_side
                # Player
                px = offset_x + cam_pos[0] * scale
                ...
                if time.time() < self.jump_time:
                    ...
                    player_fill = 'cyan'  # Defend flash
                self.canvas.create_oval(px - player_size, py - player_size, px + player_size, py + player_size, fill=player_fill)
                # Direction (horizontal in side view)
                dir_len = 10
                dx = sin(yaw) * dir_len
                self.canvas.create_line(px, py, px + dx * scale, py, fill='white', width=2)
                # Eye dot
                eye_dx = sin(yaw) * (player_size - 1)
                self.canvas.create_oval(px + eye_dx - 1, py - 1, px + eye_dx + 1, py + 1, fill='black')
                # Bosses
                for boss in self.bosses:
                    bx = offset_x + boss['pos'][0] * scale
                    ...
                    if time.time() < boss.get('anim_time', 0):
                        boss_fill = 'orange'  # Boss attack flash
                    self.canvas.create_oval(bx - boss_size, by - boss_size, bx + boss_size, by + boss_size, outline=boss_fill)
                    # Health ring
                    health_ratio = boss['health'] / boss['max_health']
                    self.canvas.create_arc(bx - boss_size, by - boss_size, bx + boss_size, by + boss_size, start=0, extent=360*health_ratio, style='arc', outline='red', width=2)
                    # Glow
                    if boss['health'] < boss['max_health'] / 2:
                        glow_color = elem_glow_map.get(boss['element'], 'white')
                        self.canvas.create_oval(bx - boss_size - 2, by - boss_size - 2, bx + boss_size + 2, by + boss_size + 2, outline=glow_color)
                # Projectiles etc.
                for proj in self.projectiles:
                    px = offset_x + proj['pos'][0] * scale
                ...
                for pickup in self.pickups:
                    ...
                    self.canvas.create_oval(px - 3, py - 3, px + 3, py + 3, fill='gold')
                    # Bloom particles
                    for _ in range(5):
                        bx = px + random.uniform(-5, 5)
                        by = py + random.uniform(-5, 5)
                        self.canvas.create_oval(bx - 1, by - 1, bx + 1, by + 1, fill='yellow')
                return
        ...
        if render_player:
            ...
            if p_proj:
                ...
                if time.time() < self.jump_time:
                    ...
                    player_fill = 'cyan'  # Defend flash
                self.canvas.create_oval(p_proj[0] - player_size, p_proj[1] - player_size, p_proj[0] + player_size, p_proj[1] + player_size, fill=player_fill)
                # Direction indicator
                dir_len = 10
                dx = sin(yaw) * dir_len
                dz = cos(yaw) * dir_len
                d_proj = proj_func([cam_pos[0] + dx, cam_pos[1], cam_pos[2] + dz], cam_pos, yaw, pitch)
                if d_proj:
                    self.canvas.create_line(p_proj[0], p_proj[1], d_proj[0], d_proj[1], fill='white', width=2)
                # Player glow if elements
                if sum(self.elements.values()) > 0:
                    self.canvas.create_oval(p_proj[0]-player_size-2, p_proj[1]-player_size-2, p_proj[0]+player_size+2, p_proj[1]+player_size+2, outline='white')

        # Pickups in 3D
        for pickup in self.pickups:
            p_proj = proj_func(pickup['pos'], cam_pos, yaw, pitch)
            if p_proj:
                size = 3 / max(1, p_proj[2] * 0.05)
                self.canvas.create_oval(p_proj[0] - size, p_proj[1] - size, p_proj[0] + size, p_proj[1] + size, fill='gold')
                # Bloom particles
                for _ in range(5):
                    bx = p_proj[0] + random.uniform(-5, 5)
                    by = p_proj[1] + random.uniform(-5, 5)
                    self.canvas.create_oval(bx - 1, by - 1, bx + 1, by + 1, fill='yellow')
