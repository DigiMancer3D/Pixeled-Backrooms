# bSIM_068.py (CORRECTED VERSION - Splash Screen Fixed)
# Run with: python3 bSIM_068.py
...
class bSIM:
    def __init__(self):
        ...
        # Show loading screen (NOW ROBUST)
        ...
        # Populate loot_rows dynamically
        self.loot_rows = [
        ...
        self.chunk_size = 500
        self.loaded_chunks = {}
        self.generate_tone(440, 0.2, 'ui_click.wav')
        self.generate_noise(0.5, 'ui_hover.wav')
        self.generate_explosion_noise('explosion.wav')
        self.generate_drink_sound('drink.wav')
    ...
    def show_loading_screen(self):
        ...
        # ROBUST LOADING - catches the exact TclError you got
        if os.path.exists(splash_path):
            try:
                ...
                # Fallback if file is corrupt / unrecognized / bad format
    ...
    def hide_loading_screen(self):
        ...
        if hasattr(self, 'splash_img') and self.splash_img is not None:
            del self.splash_img  # Free memory
        self.root.update()
    # === ALL OTHER METHODS REMAIN UNCHANGED (kept identical for full functionality) ===
    ...
    def play_sound(self, filename):
        if self.audio_player:
            try:
                subprocess.Popen([self.audio_player, filename])
            except Exception as e:
                print(f"Error playing {filename}: {e}")
    def generate_explosion_noise(self, filename='explosion.wav'):
        duration = 0.8
        sample_rate = 44100
        num_samples = int(duration * sample_rate)
        data = []
        for i in range(num_samples):
            freq = 100 + 50 * (1 - i / num_samples)
            amp = 20000 * math.sin(2 * math.pi * freq * i / sample_rate) * (1 - i / num_samples)**2
            amp += random.randint(-5000, 5000)
            data.append(struct.pack('<h', int(amp)))
        with wave.open(filename, 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)
            wf.setframerate(sample_rate)
            wf.writeframes(b''.join(data))
    def generate_drink_sound(self, filename='drink.wav'):
        duration = 0.6
        sample_rate = 44100
        num_samples = int(duration * sample_rate)
        data = []
        for i in range(num_samples):
            freq = 200 + 100 * math.sin(i / 1000)
            amp = 15000 * math.sin(2 * math.pi * freq * i / sample_rate) * (1 - i / num_samples)
            data.append(struct.pack('<h', int(amp)))
        with wave.open(filename, 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)
        ...
        for root, _, files in os.walk(base_dir):
            ...
            for f in files:
                ...
                if fields['category'].lower() == 'premade user playable diy characters':
                    ...
                    self.characters.append({'name': display_name, 'preview': preview, 'full_path': full, 'direction': direction, 'rarity': rarity})
    # === ALL REMAINING METHODS (calculate_octagon, screen_to_world, world_to_screen, is_near_player, get_safe_at, get_interactive_at, clamp_to_arena, collides, load_crumbs, save_crumbs, start_new_game, reset_game_variables, generate_arena, warp_cursor_to_center, clip_cursor, is_tla_row_open, toggle_tla_row, is_mouse_in_hud, is_near_hud, on_motion, key_down, key_up, handle_hud_left_click, on_left_click, on_left_release, handle_menu_button, handle_safe_menu, handle_interactive_left, on_right_click, on_right_release, on_right_drag, handle_escape, open_safe, draw_title_screen, draw_world, get_visible_chunks, render_chunk, draw_player, draw_menu_ring, draw_safe_menu_ring, draw_settings_overlay, draw_skilltree, draw_pause_menu, draw_hud, draw_arrows, draw, spawn_safe, spawn_assist_safe, game_update, on_close, on_mouse_wheel, generate_tone, generate_noise) ===
    # (They are identical to the previously provided working code - only splash handling was updated above to prevent the exact TclError you saw)
    ...
    def generate_arena(self):
        ...
        for _ in range(4):
            ...
            if random.random() < 0.5:
                inv.append(random.choice(list(self.item_imgs.keys())))
        ...
        for _ in range(random.randint(1,5)):
            if not self.new_objs:
                ...
                obj = random.choice(fallback_objs)
            else:
                obj = random.choice(self.new_objs)
            ox = random.uniform(-self.arena_radius * 0.8, self.arena_radius * 0.8)
            oy = random.uniform(-self.arena_radius * 0.8, self.arena_radius * 0.8)
        ...
        for obs in self.obstacles:
            cx = int(obs["x"] // self.chunk_size)
            cy = int(obs["y"] // self.chunk_size)
            key = (cx, cy)
            if key not in self.loaded_chunks:
                self.loaded_chunks[key] = []
    ...
    def on_left_click(self, event):
        if self.state == "TITLE":
            if self.width - 140 < event.x < self.width - 120 and self.height - 220 < event.y < self.height - 200:
                self.character_scroll = max(0, self.character_scroll - 1)
                return
            if self.width - 115 < event.x < self.width - 95 and self.height - 220 < event.y < self.height - 200:
                self.character_scroll += 1
                return
            if 100 <= event.x <= 1100 and 700 <= event.y <= 900:
                ...
                for vi, char in enumerate(vis_chars):
                    ...
                    col = vi % num_cols
                    roww = vi // num_cols
                    cx = 150 + col * col_width
                    cy = 750 + roww * row_height
                    if cx - 50 < event.x < cx + 50 and cy - 50 < event.y < cy + 50:
                        ...
                        return
                    dot_y = cy + 80
                    centers = [cx - 20, cx, cx + 20]
                    damages = ["Heat/Burn", "Cold/Freeze", "Physical/Bleeding"]
                    fills = ["#ff0000", "#4488ff", "#ffffff"]
                    for j, dcx in enumerate(centers):
                        if math.hypot(event.x - dcx, event.y - dot_y) < 13:
                            self.selected_damage_type = damages[j]
                            self.selected_char_color = fills[j]
                            self.selected_showcase = i
                            path = char['full_path']
                            self.custom_preview = char['preview']
                            self.custom_image = tk.PhotoImage(file=path).subsample(2)
    ...
    def draw_title_screen(self):
        self.canvas.create_text(self.width / 2, 120, text="TOAST Engine", font=("Courier", 72, "bold"), fill="#ffcc00")
        self.canvas.create_text(self.width / 2, 210, text=":Select a Character:", font=("Arial", 18, "bold"), fill="#aaaaaa")
        sections = [
        ...
        for sx, sy, ex, ey, ctype, base_col, name, damage_type in sections:
            selected = self.selected_char_type == ctype or (self.selected_char_type in ["witch", "necromancer", "elemental"] and ctype == "diy")
        ...
        if self.selected_char_type:
            ...
            if self.selected_char_type:
                selected_index = ["witch", "necromancer", "elemental", "diy"].index(self.selected_char_type)
                selected_cx = [(70 + 290)/2, (330 + 550)/2, (590 + 810)/2, (850 + 1070)/2][selected_index]
                self.canvas.create_text(selected_cx, 560 + 25, text=self.selected_damage_type, font=("Arial", 14, "bold"), fill=self.selected_char_color if self.selected_char_color != "#222222" else "#00aa00" if self.selected_damage_type != "Decay/Rot" else "#aa0000")
        if self.selected_char_type:
        ...
        for vi in range(start_idx, end_idx):
            ...
            cx = base_x + col * col_width
            cy = base_y + roww * row_height
            char = self.characters[i]
            if i == self.selected_showcase:
                self.canvas.create_rectangle(cx - 50, cy - 50, cx + 50, cy + 50, outline="#ffff00", width=4)
            self.canvas.create_image(cx, cy, image=char['preview'])
            self.canvas.create_text(cx, cy + 60, text=char['name'], font=("Arial", 10, "bold"), fill="#ffffff")
    ...
    def draw_player(self):
        ...
        if self.player['type'] in ['witch', 'necromancer', 'elemental'] and self.player['flip_progress'] >= 0:
        ...
        if self.player.get("custom_image"):
            if self.custom_image_L and self.custom_image_R:
                if self.player['type'] in ['witch', 'necromancer', 'elemental'] and self.player['flip_progress'] >= 0:
                    if p < 0.5:
                        img = self.custom_image_R if self.player['start_factor'] > 0 else self.custom_image_L
                        scale_x = (1 - 2*p)
                    else:
                        img = self.custom_image_R if self.player['target_factor'] > 0 else self.custom_image_L
                        scale_x = (2*p - 1)
                else:
                    img = self.custom_image_R if scale_x > 0 else self.custom_image_L
            else:
                img = self.custom_image
            if img:
                item = self.canvas.create_image(sx, sy, image=img)
                self.canvas.scale(item, sx, sy, scale_x, 1)
            else:
    ...
    def draw_pause_menu(self):
        ...
        if self.monitors:
            monitor_name = self.monitors[self.current_monitor]['name']
            btns.append((f"Monitor: {monitor_name}", 570))
        for i, (text, y) in enumerate(btns):
            ...
            self.canvas.create_rectangle(self.w_scale * 420, self.h_scale * y, self.w_scale * 780, self.h_scale * (y+40), fill="#333333", outline=outline, width=width)
        ...
        if self.state == "GAME" and self.player and not self.menu_open and not self.safe_menu_open and not self.pause_open and not self.settings_open and not self.skilltree_open:
            ...
            if self.fpv_mode:
                ...
                if 'w' in self.keys or 's' in self.keys or 'a' in self.keys or 'd' in self.keys:
                    ...
                    if self.player["type"] in ["witch", "necromancer", "elemental"] and target_factor != self.player['current_factor'] and self.player['flip_progress'] < 0 and time.time() - self.player['last_flip'] > 3.14:
    ...
    def on_mouse_wheel(self, event):
        ...
        self.generate_tone(880, 0.1)
        self.play_sound('temp_tone.wav')
    def generate_tone(self, freq=440, duration=0.5, filename='temp_tone.wav'):
        sample_rate = 44100
        num_samples = int(duration * sample_rate)
        data = []
        for i in range(num_samples):
            amp = 32767 * math.sin(2 * math.pi * freq * i / sample_rate) * (1 - i / num_samples)
            data.append(struct.pack('<h', int(amp)))
        with wave.open(filename, 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)
            wf.setframerate(sample_rate)
            wf.writeframes(b''.join(data))
    def generate_noise(self, duration=1.0, filename='temp_noise.wav'):
        sample_rate = 44100
        num_samples = int(duration * sample_rate)
        data = [struct.pack('<h', random.randint(-32768, 32767)) for _ in range(num_samples)]
        with wave.open(filename, 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)
            wf.setframerate(sample_rate)
