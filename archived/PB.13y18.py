...
class MapMaker:
    ...
    def move_tab_right(self):
        current = self.notebook.index("current")
        if current < len(self.maps) - 1:
            self.notebook.insert(current + 2, self.notebook.select())
    ...
    def toggle_view(self):
        ...
        if view == 'Z=Z':
            self.maps[self.current_index]['view'] = 'Y=Z'
            self.mapmenu.entryconfig(self.view_index, label="Set Top-View")
        else:
            self.maps[self.current_index]['view'] = 'Z=Z'
    ...
    def limit_ai_select(self, limit):
        ...
        if len(sel) > limit:
            ...
            if new:
                ...
                messagebox.showwarning("Limit", "Max selections reached")
    ...
    def select_arc(self, event):
        ...
        if selection:
            item_text = self.arc_list.get(selection[0])
    ...
    def apply_size(self, index):
        ...
        if new_width != map_data['width'] or new_height != map_data['height']:
            ...
            new_grid['range'] = 0.0
            new_grid['sun'] = 'NA'
            new_grid['earmark'] = 'Normal'
            new_grid['title_card'] = 'OFF'
    ...
    def _save_map_content(self, map_data, var_dict):
        ...
        props = [
            ...
            if not (cell['symbol'] == ' ' and cell['color'] == '#000000' and cell['texture'] == '' and cell['name'] == '' and cell['value'] == 0 and cell['depth'] == 1 and cell['height'] == 1 and cell['3d'] == 0 and cell['range'] == 0.0 and cell['sun'] == 'NA' and cell['earmark'] == 'Normal' and cell['title_card'] == 'OFF' and t_str == '' and o_str == '')
        ]
    ...
    def load_tmap(self, file):
        ...
        if lines:
            ...
            view_match = re.search(r' view (Y=Z|Z=Z)', header)
            if view_match:
    ...
    def get_map_image(self, index, opacity=1.0):
        ...
        for y in range(h):
            for x in range(w):
                cell = map_data['grid'][y, x]
                fill_color = cell['color'] if cell['color'] and cell['color'] != '' else '#000000'
                draw.text((x * self.cell_size + self.cell_size/2, y * self.cell_size + self.cell_size/2), cell['symbol'], fill=fill_color, font=font, anchor="mm")
        # sun and pin rects
        if map_data['sunrise']:
    ...
    def preview_dictionary(self):
        ...
        text_widget = tk.Text(preview_win, wrap=tk.NONE)
        text_widget.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        vscroll = tk.Scrollbar(preview_win, orient=tk.VERTICAL, command=text_widget.yview)
        vscroll.pack(side=tk.RIGHT, fill=tk.Y)
        hscroll = tk.Scrollbar(preview_win, orient=tk.HORIZONTAL, command=text_widget.xview)
        hscroll.pack(side=tk.BOTTOM, fill=tk.X)
        text_widget.config(yscrollcommand=vscroll.set, xscrollcommand=hscroll.set)
        map_names = [map_data['name'] for map_data in self.maps]
        import_header = 'import {' + ', '.join(f'"{n}"' for n in map_names) + '}\n'
        text_widget.insert(tk.END, import_header + '\n')
        for arc in self.arcs:
            arc_copy = copy.deepcopy(arc)
            arc_copy['start_msg'] = "***" + arc_copy['start_msg'] + "***"
            arc_copy['confirm_msg'] = "***" + arc_copy['confirm_msg'] + "***"
            arc_line = '||'.join([arc_copy[k] for k in ['name', 'estimated', 'zone_type', 'start_msg', 'map', 'arc_data', 'confirm_msg']]) + '\n'
            text_widget.insert(tk.END, arc_line)
    ...
    def export_map_png(self, index, path_or_buf):
        ...
        if isinstance(path_or_buf, str):
            img.save(path_or_buf)
        else:
            img.save(path_or_buf, 'PNG')
        ...
        try:
            ...
            if map_data['attached_arcs']:
                for a in map_data['attached_arcs']:
                    ...
                    a_copy['start_msg'] = "***" + a_copy['start_msg'] + "***"
                    a_copy['confirm_msg'] = "***" + a_copy['confirm_msg'] + "***"
