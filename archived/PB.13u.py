...
class MapMaker:
    ...
    def setup_ui(self):
        ...
        # Drawers frame for properties, colors, and minimap
        self.drawers_frame = tk.Frame(right_frame)
        # Property vscroll and canvas (hidden initially)
        ...
        # Color vscroll and canvas (hidden initially)
        ...
        col = 0
        row = 0
        max_rows = 7 # Adjust based on number of total options needed (5x7 [WxH] is current for holding 35 phrase options)
    ...
    def on_canvas_motion(self, event):
        ...
        if self.select_start and self.select_mode:
            ...
            if not self.redraw_timer:
                self.redraw_timer = self.root.after(50, self._do_redraw)
    def _do_redraw(self):
        self.redraw_timer = None
    ...
    def generate_minimap(self):
        ...
        # Simple grid layout for disconnected maps
        ...
        for i in range(len(self.maps)):
            ...
            bg_rect = self.minimap_canvas.create_rectangle(x - 1, y - 1, x + box_size + 1, y + box_size + 1, fill='black') # Border simulation
    ...
    def get_blended_image(self, index):
        if index == 0 or self.blending_sliders[index].get() == 0:
            return self.get_map_image(index, 1.0)
        value = self.blending_sliders[index].get() / 100.0
    ...
    def preview_dictionary(self):
        # TODO: implement preview for dictionary.
        pass
    def main_menu(self):
    ...
    def update_blending_sliders(self):
        ...
        self.blending_sliders = []
        for i in range(len(self.maps)):
            slider = tk.Scale(self.blending_frame, orient=tk.HORIZONTAL, from_=0, to=97, resolution=1, label=f"Blend Map {i+1}", length=50)
