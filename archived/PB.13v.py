...
class MapMaker:
    def __init__(self, root):
        ...
        # Blending sliders
        self.blending_vars = []  # one IntVar per map
    ...
    def setup_ui(self):
        ...
        self.symbol_list.bind("<Button-3>", lambda e: messagebox.showinfo("Symbols List", "Select a symbol to place on the map or Select '--' to view symbol properties."))
        ...
        self.arc_list.bind("<Button-3>", lambda e: messagebox.showinfo("Arcs List", "Select an arc to edit or click Add New."))
        ...
        self.property_canvas.bind("<Button-3>", lambda e: messagebox.showinfo("Object Properties Drawer", "Edit properties of selected cells or last dropped symbol."))
        ...
        self.color_canvas.bind("<Button-3>", lambda e: messagebox.showinfo("Color Picker Drawer", "Customize text and selection colors."))
        ...
        self.minimap_canvas.bind("<Button-3>", lambda e: messagebox.showinfo("Mini-Map Drawer", "View mini-maps."))
        ...
        tk.Checkbutton(self.minimap_footer_frame, text="Load Random", variable=self.randomize_at_load).pack(side=tk.LEFT)
        tk.Button(self.minimap_footer_frame, text="Refresh", command=self.generate_minimap).pack(side=tk.LEFT)
        tk.Button(self.minimap_footer_frame, text="Close", command=self.toggle_minimap_drawer).pack(side=tk.LEFT)
        ...
        arc_inner_frame.bind("<Button-3>", lambda e: messagebox.showinfo("Arc Data", "Edit arc fields to build plot arcs."))
        ...
        arc_name_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        arc_start_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        arc_map_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        map_type_combo.bind("<Button-3>", lambda e: messagebox.showinfo("Map Type Selector", "Select the type of map: File (Import) or On-The-Fly-Creation (Generate)"))
        ...
        arc_confirm_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        self.input_gen_list.bind("<Button-3>", lambda e: messagebox.showinfo("Script Generator Select", "Select a script type to fill in a forum to inject into the Arc Data."))
        ...
        for phrase in phrases:
            ...
            btn.bind("<Button-3>", lambda e, p=phrase: messagebox.showinfo("Phrase", f"Inject '{p}' into arc data"))
    ...
    def update_est_picker(self):
        ...
        for l in lengths:
            ...
            entry.bind("<FocusIn>", lambda e: self.select_all(e))
    ...
    def add_map_tab(self, name):
        ...
        width_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        height_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        name_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        maker_entry.bind("<FocusIn>", lambda e: self.select_all(e))
        ...
        self.blending_vars.append(tk.IntVar(value=0))
    ...
    def on_tab_change(self, event):
        if self.notebook.tabs():
            ...
            # Restore blending slider value for this map
            if self.current_index < len(self.blending_sliders):
                self.blending_sliders[self.current_index].set(self.blending_vars[self.current_index].get())
    ...
    def on_canvas_motion(self, event):
        ...
        if self.select_start and self.select_mode:
            ...
            if not self.redraw_timer:
                self.redraw_timer = self.root.after(16, self._do_redraw)
        ...
        if self.select_start and self.select_mode:
            ...
            if maxx - minx == 1 and maxy - miny == 1:
                ...
                if not self.color_open:
                    ...
                    self.property_vscroll.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def handle_right_click(self, event):
        ...
        if 0 <= x < map_data['width'] and 0 <= y < map_data['height']:
            ...
            messagebox.showinfo("Canvas Info", "This is the map canvas. Left-click to place symbols, right-click to remove symbols.")
    ...
    def toggle_color_drawer(self):
        if self.color_open:
            ...
            self.drawers_frame.update_idletasks()
            self.right_frame.update_idletasks()
            if self.selected_x is not None or self.selected_region:
                self.minimap_hscroll.pack_forget()
                self.minimap_vscroll.pack_forget()
                self.minimap_canvas.pack_forget()
            ...
            self.drawers_frame.update_idletasks()
            self.right_frame.update_idletasks()
            self.color_vscroll.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def toggle_minimap_drawer(self):
        if self.minimap_open:
            ...
            self.drawers_frame.update_idletasks()
            self.right_frame.update_idletasks()
            self.minimap_hscroll.pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def generate_minimap(self):
        ...
        for i in range(len(self.maps)):
            ...
            bg_rect = self.minimap_canvas.create_rectangle(x - 1, y - 1, x + box_size + 1, y + box_size + 1, fill='black')
            rect = self.minimap_canvas.create_rectangle(x, y, x + box_size, y + box_size, fill='yellow')
            self.minimap_canvas.create_text(x + box_size / 2, y + box_size / 2, text=str(i+1))
            self.minimap_rects.append((i, rect, bg_rect))
            col += 1
            if col >= cols:
                col = 0
                row += 1
    ...
    def save_arc_as(self):
        ...
        arc['start_msg'] = "***" + arc['start_msg'] + "***"
        arc['confirm_msg'] = "***" + arc['confirm_msg'] + "***"
    ...
    def load_arc_file(self, add_to_list=False, auto_select=False):
        ...
        if file:
            ...
            parts = line.split('||')
            if len(parts) == 7:
                start_msg = parts[3].strip("***")
                confirm_msg = parts[6].strip("***")
                arc = {
                    'name': parts[0],
                    'estimated': parts[1],
                    'zone_type': parts[2],
    ...
    def _save_map_content(self, map_data, var_dict):
        ...
        props = [
            f'{cell["symbol"]}[\"{cell["color"]}\";\"{cell["name"]}\";\"{cell["texture"]}\";{cell["sun"] if cell["sun"] != "NA" else ""}({x},{cell["3d"]},{y},{cell["depth"]},{cell["height"]},{cell["range"]}){t_str}{o_str}{cell["value"]}]'
            for y in range(map_data['height'])
            for x in range(map_data['width'])
            ...
            if not (cell['symbol'] == ' ' and cell['color'] == '#000000' and cell['texture'] == '' and cell['name'] == '' and cell['value'] == 0 and cell['depth'] == 1 and cell['height'] == 1 and cell['3d'] == 0 and cell['range'] == 0.0 and cell['sun'] == 'NA' and t_str == '' and o_str == '')
        ]
        props_str = ' ' + ' '.join(props) if props else ''
        arcs_str = ''
        if map_data['attached_arcs']:
            for a in map_data['attached_arcs']:
                a['start_msg'] = "***" + a['start_msg'] + "***"
                a['confirm_msg'] = "***" + a['confirm_msg'] + "***"
    ...
    def load_tmap(self, file):
        ...
        if lines:
            ...
            if len(footer_parts) > 4:
                ...
                for sym, color, name_, texture, sun, x, threed, y, d, h, r, t_str, o_str, val in props:
                    x = int(x)
                    y = int(y)
    ...
    def parse_props(self, props_str):
        ...
        for block in blocks:
            if not block.strip():
                continue
            match = re.match(r'(.)\["([^"]*)";"([^"]*)";"([^"]*)";?([A-Z]{2})?\((\d+),(\d+),(\d+),(\d+),(\d+),?(\d*\.?\d*)?\)((\+t\(\d+,\d+\))?(\+o\(\d+,\d+\))?(\d*))', block.strip())
            if match:
                props.append(match.groups())
        return props
    ...
    def save_and_exit(self):
        if self.user_active > 0 and not self.dirty:
            self.on_close()
            return
        if messagebox.askyesno("Auto Save", "Perform auto-save before exit?"):
    ...
    def get_map_image(self, index, opacity=1.0):
        ...
        w = map_data['width']
        h = map_data['height']
        img = Image.new('RGBA', (w * self.cell_size, h * self.cell_size), (255, 255, 255, 0))
        draw = ImageDraw.Draw(img)
        font = ImageFont.load_default()
        for i in range(w + 1):
            draw.line([(i * self.cell_size, 0), (i * self.cell_size, h * self.cell_size)], fill='gray')
        for j in range(h + 1):
            draw.line([(0, j * self.cell_size), (w * self.cell_size, j * self.cell_size)], fill='gray')
        for y in range(h):
            for x in range(w):
                cell = map_data['grid'][y, x]
                draw.text((x * self.cell_size + self.cell_size/2, y * self.cell_size + self.cell_size/2), cell['symbol'], fill=cell['color'], font=font, anchor="mm")
        if map_data['sunrise']:
    ...
    def get_blended_image(self, index):
        if index == 0 or self.blending_vars[index].get() == 0:
            return self.get_map_image(index, 1.0)
        value = self.blending_vars[index].get() / 100.0
    ...
    def update_blending_sliders(self):
        ...
        self.blending_sliders = []
        for i in range(len(self.maps)):
            var = self.blending_vars[i]
            slider = tk.Scale(self.blending_frame, orient=tk.HORIZONTAL, from_=0, to=97, resolution=1, label=f"Blend Map {i+1}", length=50, variable=var)
    ...
    def hide_maps(self):
        ...
        self.main_vertical_paned.sash_place(0, 0, 30)
        self.maps_hidden = True
        self.update_map_menu_states()
    ...
    def get_pins(self):
        if messagebox.askokcancel("Get Pins", "After clicking 'Ok', click any two cells. The first will pin the map below at that spot. The second will pin the map above to that spot."):
    ...
    def close_tab(self, index):
        ...
        self.deleted_maps.append((index, copy.deepcopy(map_data), copy.deepcopy(self.var_dicts[index]), copy.deepcopy(self.undo_stacks[index]), copy.deepcopy(self.redo_stacks[index])))
    ...
    def undo_map_delete(self):
        if self.deleted_maps:
            pos, map_data, var_dict, undo_stack, redo_stack = self.deleted_maps.pop()
            ...
            self.blending_vars.insert(pos, tk.IntVar(value=0))
            frame = tk.Frame(self.notebook)
    ...
    def export_map_png(self, index, path_or_buf):
        ...
        if isinstance(path_or_buf, str):
            ...
            if map_data['attached_arcs']:
                arc_lines = ['||'.join([a[k] for k in ['name', 'estimated', 'zone_type', 'start_msg', 'map', 'arc_data', 'confirm_msg']]) for a in map_data['attached_arcs']]
                arcs_str = ';arcs::' + ';'.join(arc_lines)
            txt_content = header + ' ' + footer + props_str + arcs_str
    ...
    def export_full_dict(self):
        ...
        try:
            ...
            with zipfile.ZipFile(path, 'w') as zf:
                ...
                for arc in self.arcs:
                    csv_buf = StringIO()
                    self.export_arc_csv(arc, csv_buf)
                    csv_buf.seek(0)
                    zf.writestr(f'arc/{arc["name"]}.csv', csv_buf.getvalue())
        finally:
            self.root.config(cursor='')
    ...
    def on_openings_change(self, event):
        index = self.current_index
        self.maps[index]['openings'] = self.var_dicts[index]['openings_var'].get()
        if self.minimap_open:
            self.generate_minimap()
